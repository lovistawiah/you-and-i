import { Link } from 'react-router-dom'
import logoSvg from '../svg/logo.svg'
import '../styles/add-email.css'
const AddEmail = () => {
  return (
    <section className="container">
      <div className="skip-div">
        {/* add next page to here */}
        <Link className='skip-link'>Skip</Link>
      </div>
      <section className='logo-box'>
        <img src={logoSvg} alt="logo of you and I" />
      </section>
      <section className="name-email-message">
        <section className="company-name">You and I</section>
        <section className="email-message">Add Email Address</section>
      </section>
      <form className='form'>
        <input type="text" name="" id="" placeholder='Email Address' className='form-input' />
        <button className='form-button'>Add email</button>
      </form>
    </section>
  )
}
export default AddEmail